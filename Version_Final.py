...
def graficas():
    ...
    button2 = tk.Button(window, text='Graficar', width=25, command=Grafica)
    button2.pack()
...
def Grafica():
    window.destroy()
    df = pd.read_excel(fileName, sheet_name='PRINCIPAL').dropna(how='all')
    df['Año/Mes'] = df['Fe. Inic. Orden'].dt.to_period('M')
    sns.countplot( x='Año/Mes', hue="Estatus Refacción", data=df)
    plt.subplots_adjust(left=0.1, bottom=0.3)

    Filtrar = df[df[('Estatus Refacción')] == 'Completo']
    #Correcto = Filtrar.drop_duplicates(subset=['# de Sap.'])
    numero = Filtrar['Costo en SAP Refacción'].sum()
    Nuevo = df[df['Estatus Refacción']=='Completo']['Estatus Refacción'].count()

    Filtrar2 = df[df[('Estatus Refacción')] == 'Parcial']
    #Correcto2 = Filtrar2.drop_duplicates(subset=['# de Sap.'])
    numero2 = Filtrar2['Costo en SAP Refacción'].sum()
    Nuevo2 = df[df['Estatus Refacción']=='Parcial']['Estatus Refacción'].count()

    Filtrar3 = df[df[('Estatus Refacción')] == 'Transito']
    #Correcto3 = Filtrar3.drop_duplicates(subset=['# de Sap.'])
    numero3 = Filtrar3['Costo en SAP Refacción'].sum()
    Nuevo3 = df[df['Estatus Refacción']=='Transito']['Estatus Refacción'].count()
    
    textstr = 'Completo= Cantidad: ' + str(Nuevo) + ' Total: ' + str(numero) + '\nParcial= Cantidad: '+ str(Nuevo2) + ' Total: ' + str(numero2)+ '\nTransito= Cantidad: ' + str(Nuevo3) + ' Total: ' + str(numero3)
    plt.gcf().text(0.5, 0.01, textstr, fontsize=8)

    
    axButton1 = plt.axes([0.4,0.1,0.1,0.1])
    btn1 = Button(ax=axButton1, label='Regresar')
    btn1.on_clicked(Button35)
    plt.show()
...
def Tabla2():
    ...
    #Filtrar = df[df[('Estatus Refacción')] == 'Parcial']
    #Correcto = Filtrar.drop_duplicates(subset=['# de Sap.'])
    numero = Completo['Costo en SAP Refacción'].sum()
    Nuevo = df[df['Estatus Refacción']=='Parcial']['Estatus Refacción'].count()
...
def Tabla3():
    ...
    #Filtrar = df[df[('Estatus Refacción')] == 'Transito']
    #Correcto = Filtrar.drop_duplicates(subset=['# de Sap.'])
    numero = Completo['Costo en SAP Refacción'].sum()
    Nuevo = df[df['Estatus Refacción']=='Transito']['Estatus Refacción'].count()
    cosa = Completo[['Ubicación Técnica','# de Sap.','Descripción', 'Cantidad', 'Ots','# Pedido', 'Fe. Entrega', 'Proveedor', 'Costo en SAP Refacción','Estatus Refacción']]
    Txtbox.insert('end', '\n Cantidad: '+str(Nuevo)+ ' Total: ' +str(numero))
    Txtbox.insert(1.0, cosa)
    window5.mainloop()
